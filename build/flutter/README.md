...
* `scrollfit` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `scrollfit` - executable or bundle name on disk.
* `ScrollFit` - display name shown in window titles and about app dialogs.
* `com.manumi` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
